...
#' @param selection a character string indicating how to identify the clusters:
...
#' @param lambda the threshold of the (log-) likelihood ratio, above which the
...
Kulldorff2001 <- function(observed, theoretical, ngkd, loglambda=TRUE)
{
    ...
    if (loglambda) {
        resu[,1] <- log(resu[,1])
        names(resu) <- c("loglambda", "radius","time","Nobs","Nth")
    } else {
        names(resu) <- c("lambda", "radius","time","Nobs","Nth")
    }
...
#' @rdname ngkd
#' @export
identifyClusters <- function(spixdf, ngkd, selection=c("nclust","lambda"),
                             nclustMax=nrow(spixdf), lambda=NULL, plot=TRUE)
{
    ...
    if (nclustMax>1) {
        ...
        for (i in 1:(nclustMax-1)) {
            ...
            nob[i+1] <- spixdf2[[4]][ma]
            nth[i+1] <- spixdf2[[5]][ma]
            ind[i+1] <- index2[ma]
        }
    }

    df <- data.frame(index=ind, radius=dia, time=tim, lam=lam, Nobs=nob, Nth=nth)
